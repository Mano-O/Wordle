...
pygame.init() # initializing py game
running = True
fps = 60
grey = (178, 190, 181)
green = (99, 201, 62)
...
width = 
height = 
screen = pygame.display.setmode((width, height)) # initialize a window
clock = pygame.time.clock() # to set the speed of the game to be equal on all monitors
pygame.display.set_caption("Wordle M.I.A") # game title

while running: # game loop
    clock.tick(fps) # sets the speed
    screen.fill(grey)

    for event in pygame.event.get(): # to exit the loop
